    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                ts_de = report_buf[1];
                DebugPrint("Dequeue. Queue length: {0:d}. Packet ID: {1:X2}. Timestamp: {2:X2}. Lag to dequeue: {3:s}. Lag between packets (expect 15ms): {4:s}" +
                    reports.Count + report_buf[0].ToString() + report_buf[1] + System.DateTime.Now.Subtract(rep.GetTime()) + rep.GetTime().Subtract(ts_prev), DebugType.THREADING);
                ts_prev = rep.GetTime();
            }
